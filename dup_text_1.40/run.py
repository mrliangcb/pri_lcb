...
import os
@app.route('/dup_check', methods=['POST'])  # 用法 r = requests.post(url, files=files)
def dup_check():
    print('request.method是什么:', request.method)
    k_gram = 13
    # receive = request.get_data()
    # print('receive是什么:', receive)
    if request.method == 'POST':
        ...
        path1='./download/' + str(t) + '_' + f1_name
        path2='./download/' + str(t) + '_' + f2_name
        f1.save(path1)
        f2.save(path2)
        ...
        return render_template('testHtml.html', name1=f1.filename, name2=f2.filename, time=time_, dup_check=dp_c,
                               dup_text=dup_text, dup_dic=dup_dic,doc1_str=doc1_str,doc2_str=doc2_str,doc1_wrap=doc1_wrap,doc2_group_=doc2_group_)  # , name=f1.filename  这些变量发送到这个页面之后，这个页面不用特意接收，直接用就行，就像全局变量一样
        # 返回参数
        # return json.dumps([time_,dup_text,doc1_str,doc2_str,doc1_wrap,doc2_group_])
