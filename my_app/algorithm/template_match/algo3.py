...
def main(source_file, template_doc, source_isdoc, tem_isdoc):
    ...
    sou_li_str_list=[]
    sou_li_str_dic={}
    for i,j in enumerate(sour_likely_heading_list):
        sou_li_str_list.append(j.str_)
        if not sou_li_str_dic.get(j.str_,None):
            sou_li_str_dic[j.str_]=i

    for i,j in enumerate(tem_heading_obj_list):
        if j.flag==-2:
            if sou_li_str_dic.get(j.str_,None): #如果查到有
                # tem的-2改成1
                # tem的global -2改成1
                # source global 的2改成1
                # source like 2改成1
                ...
                sou_index=sou_li_str_dic[j.str_]
