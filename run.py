...
def check_str(doc1_str,doc2_str,k=13):
    ...
    for i in range(len(doc1_wrap)):
        ...
        # 在给doc2分组 成tuple
    print('doc2_group是什么',doc2_group)
    s = 0
    e = 0
    doc2_group_ = []
...
def dup_check():
    ...
    if len(args_dic['doc1'])<13 or len(args_dic['doc2']) <13:
        return jsonify(str('错误 : 文本长度太短，输文本入长度要求大于13'))
    ...
    result5 = render_template('add_href_doc2.html', doc2_group_=doc2_group_, doc2_str=doc2_str)
    result6=time_
    result7=doc1_str
    result=doc2_str
    # return result4
